...
- 실제 기존 페이지와 수정 페이지를 고화질로 미리 보고 애매한 항목만 확인
- iPad에서도 Apple이 만든 한글 CID 글꼴 PDF를 전용 Adobe CMap으로 빠짐없이 렌더링
- 필기가 있는 삭제 예정 페이지만 미리 보고 자동 삭제, 수정 PDF의 새 페이지와 직접 매칭, 문서 맨 뒤 보관 중 선택
- 필기가 없는 애매한·삭제 예정 페이지는 확인 없이 삭제하고 수정 PDF의 새 페이지로 교체
- 같은 PDF 쪽을 중복 참조하는 빈 GoodNotes 페이지는 제거하고, 필기가 있는 별도 페이지만 보존
- 크기가 다른 수정 페이지도 검색 가능한 텍스트 좌표를 유지한 채 기존 필기 캔버스에 정렬
- 수정한 PDF 배경의 오래된 검색 좌표 캐시만 제거해 GoodNotes에서 새로 색인
